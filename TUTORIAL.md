...
You're good to go! Execute `pulse run final.py` and go to the address given by the webserver on the right, most likely http://localhost:5173, to see the final app.
...
Copy/paste this code into a file, for example `tutorial.py`, and run it using `pulse run tutorial.py`. Same as above, go to http://localhost:5173 (or the address chosen by the webserver, visible in the right pane with `pulse run`) to see the app. You should see "Welcome to Pulse!" in the top-left corner.
