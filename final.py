...
# Mimic a database with in-memory storage
...
class Database:
    ...
    async def save_tasks(self, tasks: list[Task]):
    ...
    async def load_task_by_id(self, task_id: str) -> Optional[Task]:
...
class TaskManagerState(ps.State):
    ...
    async def tasks(self) -> list[Task]:
        ...
        self.query_calls += 1
        print(f"Loading all tasks (call #{self.query_calls})...")
        return await DB.load_tasks()
    ...
    async def categories(self) -> list[Category]:
        """Query to load all tasks."""
        self.query_calls += 1
        print(f"Loading all categories (call #{self.query_calls})...")
        return await DB.load_categories()

    @categories.key
    def _categories_key(self):
        return ("categories",)
    ...
    def filtered_tasks(self) -> list[Task]:
        ...
        return [
            t for t in self.tasks.data if t.category_id == self.selected_category_id
        ]
    ...
    def selected_category(self) -> Optional[Category]:
        ...
        if not self.selected_category_id:
            return None
        return next(
            (c for c in self.categories.data if c.id == self.selected_category_id), None
        )
    ...
    def new_task_category(self) -> Optional[Category]:
        ...
        return next(
            (c for c in self.categories.data if c.id == self.new_task_category_id), None
        )

    def get_category_by_id(self, category_id: str) -> Optional[Category]:
        """Helper to get category by ID."""
        return next((c for c in self.categories.data if c.id == category_id), None)
    ...
    async def add_task(self):
        ...
        new_tasks = [
            *self.tasks.data,
            Task(self.new_task_title.strip(), self.new_task_category_id),
        ]

        self.tasks.set_data([*self.tasks.data, new_task])
        self.tasks.refetch()
        ...
        for i, task in enumerate(self.tasks):
            if task.id == task_id:
                ...
                self._add_task_change(
                    self.tasks[i], "completed", str(task.completed), str(task.completed)
                )
            tasks.append(task)
        self.tasks = tasks
    ...
    def log_task_changes(self):
        ...
        task_count = len(self.tasks.data or [])
